...
def evaluate_two_models(first_model, second_model, data_first, data_second, device, criterion, y_gt1, y_gt2,
    ...
    y_first_for_table = (np.array(y_first) == y_gt1.squeeze()) * 1
    y_second_for_table = (np.array(y_second) == y_gt2.squeeze()) * 1

    table = create_contingency_table(y_first_for_table, y_second_for_table)
    return table
