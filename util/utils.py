...
def demo(model_p, model_no, model_pf, data_p, data_no, data_pf, inputs, labels, device):
    model_p.eval()
    model_pf.eval()
    model_no.eval()
    with torch.no_grad():
        for batch_idx, (batch_p, batch_no, batch_pf, example, label) in \
                enumerate(zip(data_p, data_no, data_pf, inputs, labels)):

            x_p, y_p = batch_p
            x_p, y_p = x_p.to(device), y_p.squeeze().to(device)

            x_no, y_no = batch_no
            x_no, y_no = x_no.to(device), y_no.squeeze().to(device)

            x_pf, f_pf, y_pf = batch_pf
            x_pf, f_pf, y_pf = x_pf.to(device), f_pf.to(device), y_pf.squeeze().to(device)

            lens_p = get_lengths(x_p)
            lens_pf = get_lengths(x_pf)
            lens_no = get_lengths(x_no)

            logits_p = model_p(x_p, lens_p)
            logits_no = model_no(x_no, lens_no)
            logits_pf = model_pf(x_pf, lens_pf, f_pf)

            pred_p = get_predictions(logits_p)
            probs_p = torch.sigmoid(logits_p)

            pred_pf = get_predictions(logits_pf)
            probs_pf = torch.sigmoid(logits_pf)

            pred_no = get_predictions(logits_no)
            probs_no = torch.sigmoid(logits_no)

            print(f"Example: {example}\tLabel:{'Sarcasm' if label == 1 else 'No sarcasm'}")
            print(f"Model NoPunct - {100*probs_no.detach().cpu().numpy()[0][pred_no]}% "
                  f"for {'Sarcasm' if pred_no == 1 else 'No sarcasm'}")
            print(f"Model Punct - {100 * probs_p.detach().cpu().numpy()[0][pred_p]}% "
                  f"for {'Sarcasm' if pred_p == 1 else 'No sarcasm'}")
            print(f"Model PunctFeatures - {100 * probs_pf.detach().cpu().numpy()[0][pred_pf]}% "
                  f"for {'Sarcasm' if pred_pf == 1 else 'No sarcasm'}\n")
